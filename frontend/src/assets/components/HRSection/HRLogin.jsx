...
const HRLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post(`${process.env.VITE_BASE_URL}/api/HR/login`, payload);
  ...
  return (
    <div className="flex justify-center items-center min-h-[85vh] bg-gray-100 pt-20">
      <form onSubmit={handleLogin} className="relative bg-white p-12 rounded-2xl shadow-2xl w-[32rem] space-y-8 mt-8">
        ...
        <h2 className="text-4xl font-bold mb-6 text-center text-blue-800">HR Login</h2>
        <input
          type="email"
          placeholder="Email"
          value={email}
