  ...
  return (
    <div className="flex justify-center items-center min-h-[85vh] bg-gray-100">
      <form onSubmit={handleLogin} className="relative bg-white p-12 rounded-2xl shadow-2xl w-[32rem] space-y-8 mt-8">
        <button type="button" onClick={handleClose} className="absolute top-4 right-4 text-gray-500 hover:text-gray-800 transition duration-200">
          &#10005;
        </button>

        <h2 className="text-4xl font-bold mb-6 text-center text-blue-800">Faculty Login</h2>
        <input
          type="email"
          placeholder="Email"
          value={email}
        ...
        <div className="relative mb-6">
          <input
            ...
            required
          />
          <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-3 text-gray-500 hover:text-gray-800 transition duration-200">
            {showPassword ? '🙈' : '👁️'}
          </button>
        </div>
        <button type="submit" className="w-full bg-blue-500 text-white p-4 rounded-xl text-2xl font-semibold hover:bg-blue-600 transition duration-200">
          Login
        </button>
      </form>
      ...
      {modalVisible && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg text-center">
            ...
            <button onClick={closeModal} className="mt-4 bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition duration-200">
              Close
            </button>
          </div>
        </div>
